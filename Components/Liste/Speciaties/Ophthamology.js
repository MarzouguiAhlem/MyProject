import React, { useState, useEffect } from 'react';
  ...
  export default function Ophtalmology() {
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          ...
          const response = await fetch(`http://192.168.1.129:3000/profile/${patientId}/specialties/Ophtalmology/MedicalFiles`);
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Ophtalmology Reports</Text>
        <FlatList
          data={list}
          renderItem={({ item }) => (
